...
public:
	WordDAG(Lattice *l);
	virtual uint node_begin() const { return (*lattice->we).size(); }
	virtual uint node_end() const { return (*lattice->we).size()+1; }
	virtual uint node_count() const { return (*lattice->we).size(); }
