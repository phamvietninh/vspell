...
\item Chương 4 trình bày mô hình đề nghị.
...
\item Phần phụ lục trình bày tùm lum hết.
\end{itemize}
...
Căn cứ vào độ mở miệng, co thể chia thành:
\begin{itemize}
...
đôi, trừ \textipa{/i, e/} là hai nguyên âm giòng trước, độ mở hệp và
...
vài thổ ngữ lẻ tẻ ở Nghệ Anh và Quảng Bình chỉ có 4 thanh điệu.
...
nhau. Do đó, tất sẽ không có khái niệm từ nói chung''\footnote{Nguyễn
  Kim Thản, {\em Nguyên cứu ngữ pháp tiếng Việt}. NXB GD, 1997. Trang 28}.
Chính vì tính đa dạng và phức tạp của từ mà ột số nhà ngôn ngữ học
...
là nó tồn tại, và chính sự giao lưu giữa những thực thế đó đã làm
...
ngữ âm để định nghĩa từ, xem mỗi từ trước hết là nhữngâm tiết. Với
...
đơn vị tách rời và có một ý nghĩa hiểu được}''\footnote{Nguyễn
  Kim Thản, {\em Nguyên cứu ngữ pháp tiếng Việt}. NXB GD, 1997. Trang
...
lỗi nhập liệu\footnote{typographic error}. Lỗi nhập liệu xảy ra khi gõ không
...
hảo. Việc tăng kích thước từ điển sẽ tốn kém. Hơn nữa, càng có nhiều
...
lỗi chính tả cảm ngữ cảnh~\cite{Golding95,Golding96,Golding99}
...
độc lập ngôn ngữ và là một công cụ mạnh giúp khữ nhập nhằng các văn
bản không giới hạn. 
...
công việc khó khăn vì hiện nay vẫn CG cho tiếng Việt vẫn chưa được xây
dựng. 
...
quan trọng nhất là hypernym và hyponym. A được xem là hyponym của B
(và B là hypernym của A) nếu ta có thể nói ``A là một loại đặc biệt
của B''. WordNet được tổ chức theo đơn vị là các synset. Synset
(Synonym set) là một nhóm các từ đồng nghĩa có thể dùng thay thế cho
nhau. Mỗi từ có thể thuộc nhiều synset khác nhau. Đối với mỗi từ, thì
các synset chứa từ đó được gọi là sense. Phần danh từ trong WordNet có
thể xem như một đồ thị của các synset và các liên hệ hypernym/hyponym
giữa các synset đó.  
...
\begin{algo}[Khử nhập nhằng danh từ dùng CD]
  \begin{enumerate}
...
cho trình bắt lỗi chính tả. Hạn chế quan trọng của CD khi áp dụng cho
...
TBL\footnote{Transformation-based Learning} mô hình học có giám sát,
...
chương trình khác xử lý để phát hiện hiện và sửa lỗi chính tả. Mục
...
\subsection{Mô hình kết hợp Bộ phân lớp Bayes và Danh sách quyết định}
%%% Golding 96 -> hybrid method
...
phải: phân ranh giới từ. Bài toán tìm ranh giới từ vốn đã phức tạp
...
tách từ mờ. Sau khi tách từ xong, bài toán bắt lỗi chính tả trở nên
đơn giản. Ta có thể áp dụng các kỹ thuật đã được áp dụng trên bắt lỗi
chính tả các ngôn ngữ châu Âu dễ dàng.
...
\item Dạng đơn giản, được dùng để giải quyết nhập nhằng từ đơn. Giả sử
  có một chuỗi ký tự (tương đương với chuối tiếng trong tiếng Việt)
...
Mô hình Markov ẩn phân cấp (HHMM)\footnote{Hierarchical Hidden Markov
  Model} được áp dụng trong~\cite{} để giải quyết bài toán tách từ
tiếng Trung Quốc.
...
trong Luận văn Tách từ tiếng Việt\footnote{Nguyễn Văn Toàn, Nguyễn Thi
  Minh Hằng -- Khoa Công nghệ thông tin, Khoá 97, Đại học Khoa học Tự
  nhiên} kèm với mạng nơ-ron để khử nhập nhằng. 
...
\item Mỗi loại factoids được định nghĩa như một lớp. Ví dụ, các biểu
...
lỗi\footnote{error-tolerant word segmentation}.
...
\begin{enumerate}
\item Tách đoạn. Dùng cách ký tự như \fbox{?} \fbox{!} \fbox{,}
  \fbox{:} \fbox{.}~\ldots{} để tách.
...
Tuy nhiên, với điều kiện hiện tại, khó có thể tìm được một khối lượng
ngữ liệu lớn đã được tác từ sẵn, do đó cần phải tìm giải pháp tính
được các tham số cần thiết từ ngữ liệu chưa được tách từ (nói cách
khác là ngữ liệu thô).

Thuật toán EM\footnote{Estimation Maximum} thường được áp dụng để vượt
qua khó khăn này~\cite{softcount,((()))}. Trên lý thuyết, áp dụng
thuật toán EM đảm bảo kết quả sẽ hội tụ, và kết quả ở mỗi vòng lặp sau
sẽ tốt hơn hoặc bằng kết quả của vòng lặp trước. Theo \cite{xxx}, ta
chỉ cần lặp khoảng 5-7 vòng lặp là đủ.

Do thuật toán EM cũng có nhiều hạn chế (đặc biệt là hạn chế tối ưu cục
bộ), nhiều giải pháo đã được đưa ra để khắc phụ các hạn chế
này~\cite{xxx}. 

Một giải pháp khác là áp dụng HMM\footnote{Mô hình Markov ẩn ---
Hidden Markov Model} để tìm ra ranh giới từ, sau đó áp dụng các công
thức thống kê thông thường. Giải pháp này gặp hạn chế bởi chính
HMM, vì HMM không phát huy được một số đặc trưng của tách từ mà chỉ
áp dụng như một mô hình HMM nói chung, do đó phần nào hạn chế kết
quả cuối cùng. 
